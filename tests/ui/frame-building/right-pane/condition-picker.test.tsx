...
// Popover portals its menu content to document.body — assertions on
// menuitems must query via screen, not the render container.
describe("ConditionPicker", () => {
  it("under legal mode_flavor, all condition kinds from OFFERED_CONDITIONS_BY_MODE_FLAVOR['legal'] are offered", () => {
    const on_pick = vi.fn();
    render(<ConditionPicker mode_flavor="legal" exclude_kinds={new Set()} on_pick={on_pick} />);
  ...
  it("under legal mode_flavor, 12 condition kinds are offered (full set)", () => {
    render(<ConditionPicker mode_flavor="legal" exclude_kinds={new Set()} on_pick={() => {}} />);
    fireEvent.click(screen.getByRole("button", { name: /Add condition/ }));
    const menu_items = screen.getAllByRole("menuitem");
  ...
  it("exclude_kinds filters out listed kinds", () => {
    ...
    render(<ConditionPicker mode_flavor="legal" exclude_kinds={exclude} on_pick={() => {}} />);
    fireEvent.click(screen.getByRole("button", { name: /Add condition/ }));
    const menu_items = screen.getAllByRole("menuitem");
    const labels = menu_items.map((el) => el.textContent ?? "");
  ...
  it("calls on_pick with the selected kind", () => {
    ...
    render(<ConditionPicker mode_flavor="legal" exclude_kinds={new Set()} on_pick={on_pick} />);
    fireEvent.click(screen.getByRole("button", { name: /Add condition/ }));
    const first_item = screen.getByRole("menuitem", { name: /Premise attached/ });
  ...
  it("shows 'All conditions already added' when all kinds are excluded", () => {
    ...
    render(<ConditionPicker mode_flavor="legal" exclude_kinds={all_kinds} on_pick={() => {}} />);
    fireEvent.click(screen.getByRole("button", { name: /Add condition/ }));
    expect(screen.getByText(/All conditions already added/)).toBeTruthy();
  });
});
