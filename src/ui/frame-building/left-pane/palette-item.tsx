...
/**
 * Palette item rendered as a shadcn SidebarMenuButton. When the surrounding
 * Sidebar collapses to icon mode (`data-collapsible=icon`), the button auto-
 * shrinks to a 32×32 hit-target, the label clips behind `overflow-hidden`,
 * and the supplied `tooltip` surfaces the label on hover — all built into
 * the SidebarMenuButton primitive.
 *
 * We use `asChild` so the underlying element is a real <button> that can
 * carry the draggable attribute and onDragStart handler (the palette item
 * is both clickable AND draggable onto the canvas).
 */
